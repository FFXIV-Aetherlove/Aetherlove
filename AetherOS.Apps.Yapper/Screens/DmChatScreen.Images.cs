...
/// <summary>Pictures in a DM: the attach menu (camera, photo library, a file off disk), the compose panel,
/// the bubble, the breakout viewer and the report flow. Deliberately the messenger's image UI, beat for
/// beat, because it is the same job on a different chat. The picture is the one thing here the server can
/// read, which is what lets it be screened and reported; the compose panel says so before anything is sent.</summary>
internal sealed partial class DmChatScreen
{
    private const float ImageBubbleMaxH = 240f;

    private sealed record DmImageVisual(ISharedImmediateTexture? Tex, bool Gone);

    private readonly Dictionary<Guid, DmImageVisual> _imageCache = new();
    ...
    /// <summary>Usage against the account's image budget, which the messenger and the match chats share.
    /// Pulled fresh on every compose so the line and the supporter-only expiry options are current.</summary>
    private void FetchStorage()
    {
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                _storage = await _host.GetDmImageStorageAsync().ConfigureAwait(false);
            }
            catch (Exception ex)
            {
                AetherLove.UiHost.Log.Warning(ex, "[YapperDm] image storage fetch failed.");
            }
            finally
            {
    ...
    private void DrawImageComposeOverlay(byte[]? peerKey)
    {
        ...
        var dismissed = SharedUiHelpers.DrawPageOverlayPanel("yapDmImageCompose", ImGui.GetWindowPos(),
            ImGui.GetWindowSize(), ref _composePanelH, Px(340f), innerW =>
        {
            ...
            ImGui.BeginDisabled(_imageSending || peerKey is null);
        ...
        bool supporter)
    {
        ...
        for (var i = 0; i < n; i++)
        {
            ...
            if (ImGui.InvisibleButton($"##yapDmExp{hours}", new Vector2(pillW, pillH)) && !locked)
            {
    ...
    private void SendPendingImage()
    {
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                ...
                var dto = await _host.SendDmImageAsync(
                        new SendYapperDmImageRequest(peer, upload, ExpiryHours: hours)).ConfigureAwait(false);
                _dms.Append(peer, dto, _dms.Peer(peer), countUnread: false);
                _pendingImagePath = null;
                _scrollToBottom = 1f;
            }
            catch (Exception ex)
            {
                ...
                AetherLove.UiHost.Log.Warning(ex, "[YapperDm] image send failed.");
            }
            finally
            {
                _imageSending = false;
    ...
    private Vector2 ImageThumbSize(YapperDmImageDto img, float windowWidth)
    {
    ...
    private void DrawImageMessage(YapperDmMessageDto msg, float windowWidth, bool mine, bool isGroupEnd)
    {
        var img = msg.Image!;
        var dl = ImGui.GetWindowDrawList();
        ...
        var left = mine ? cursorPos.X + windowWidth - size.X - Px(10f) : cursorPos.X + Px(10f);
        var tl = new Vector2(left, cursorPos.Y + entryDy);
        var br = tl + size;

        ImGui.SetCursorScreenPos(tl);
        var clicked = ImGui.InvisibleButton($"##yapDmImg{msg.Id:N}", size);
        var hovered = ImGui.IsItemHovered();
        if (hovered)
        {
            SharedUiHelpers.HandOnHover();
        ...
        if (expired)
        {
            ...
            CenteredImageText(dl, Loc.T("chat.image_expired"), tl.X + (size.X * 0.5f),
        ...
        else
        {
            ...
            else
            {
                ...
                CenteredImageText(dl, Loc.T("chat.image_loading"), tl.X + (size.X * 0.5f), tl.Y + (size.Y * 0.5f),
                    White(0.4f));
            }
        ...
        if (!expired && ImGui.BeginPopupContextItem($"##yapDmImgCtx{msg.Id:N}", ImGuiPopupFlags.MouseButtonRight))
        {
            ...
            if (mine && ImGui.MenuItem(Loc.T("chat.image_delete")))
            {
                FireImageDelete(img.ImageId);
                _imageCache[img.ImageId] = new DmImageVisual(null, true);
            }
            ImGui.EndPopup();
        }
        ...
        if (isGroupEnd)
        {
            ...
            dl.AddText(new Vector2(tx, br.Y + Px(3f)),
                ImGui.GetColorU32(new Vector4(0.75f, 0.75f, 0.75f, 0.4f)), timeStr);
            ImGui.SetCursorScreenPos(cursorPos + new Vector2(0f, size.Y + tsz.Y + Px(8f)));
        }
        else
        {
    ...
    /// bezel, the same breakout the yap gallery and the messenger use.</summary>
    private void DrawImageViewer()
    {
        ...
        if (ImGui.Begin($"{Loc.T("chat.image_viewer")}##yapDmImageViewer", ref open,
    ...
    private void DrawImageReportOverlay()
    {
        ...
        var dismissed = SharedUiHelpers.DrawPageOverlayPanel("yapDmImageReport", ImGui.GetWindowPos(),
            ImGui.GetWindowSize(), ref _imageReportPanelH, Px(320f), innerW =>
        {
    ...
    private void FireImageReport()
    {
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                await _host.ReportDmImageAsync(imageId, reason).ConfigureAwait(false);
                _imageReportId = Guid.Empty;
                _imageReportReason = string.Empty;
                _imageReportedToast = 4f;
            }
            catch (Exception ex)
            {
                _imageReportError = Loc.T("chat.image_err_generic");
                AetherLove.UiHost.Log.Warning(ex, "[YapperDm] image report failed.");
            }
            finally
            {
                _imageReportSubmitting = false;
    ...
    private float _imageReportedToast;
    ...
    private void FireImageDelete(Guid imageId) =>
        _ = Task.Run(async () =>
        {
            try
            {
                await _host.DeleteDmImageAsync(imageId).ConfigureAwait(false);
                PurgeImage(imageId);
            }
            catch (Exception ex)
            {
                AetherLove.UiHost.Log.Warning(ex, "[YapperDm] image delete failed.");
            }
        });
    ...
    private void StartImageFetch(Guid imageId)
    {
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                var bytes = await _host.GetDmImageAsync(imageId).ConfigureAwait(false);
                _imageCache[imageId] = bytes is { Length: > 0 }
                    ? new DmImageVisual(
                        AetherLove.Services.AvatarDiskCache.Store(ImageCacheDir, imageId.ToString("N"), bytes), false)
                    : new DmImageVisual(null, true);
            }
            catch (Exception ex)
            {
                _imageCache[imageId] = new DmImageVisual(null, true);
                AetherLove.UiHost.Log.Warning(ex, "[YapperDm] image fetch failed.");
            }
            finally
            {
    ...
    private string ImageCacheDir => Path.Combine(_caps.Storage("yapper").Directory, "DmImages");
    ...
    private void PurgeImage(Guid imageId)
    {
        ...
        {
            AetherLove.UiHost.Log.Warning(ex, "[YapperDm] purging a removed image's cache failed.");
        }
    }
    ...
    private static void CenteredImageText(ImDrawListPtr dl, string text, float cx, float cy, uint color)
    {
