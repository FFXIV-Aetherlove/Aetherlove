...
public static class VenueLocationDetector
{
    public static DetectedVenueLocation Detect()
    {
        ...
        {
            Plugin.Log.Debug(ex, "[VenueLocationDetector] World detect failed.");
        }
        ...
        if (district != HousingDistrict.Unknown)
        {
            ...
            {
                Plugin.Log.Debug(ex, "[VenueLocationDetector] Housing detect failed.");
            }
        }
    ...
    public static Region? DetectRegion()
    {
        try
        {
            ...
            var worldRow = Plugin.DataManager.GetExcelSheet<Lumina.Excel.Sheets.World>().GetRow(worldId);
            var dcRow = Plugin.DataManager.GetExcelSheet<Lumina.Excel.Sheets.WorldDCGroupType>()
        ...
        {
            Plugin.Log.Debug(ex, "[VenueLocationDetector] Region detect failed.");
            return null;
        }
    }
