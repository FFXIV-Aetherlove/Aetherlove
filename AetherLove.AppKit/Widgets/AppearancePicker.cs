...
public static class AppearancePicker
{
    public static void DrawThemeCards(float winW, float padX)
    {
        ...
        const int Cols = 2;
        var themes = ThemeService.DisplayOrder;
    ...
    public static void DrawPhoneSizeButtons(float winW, float padX, ThemeDefinition t)
    {
        ...
        {
            UiHost.Configuration.PhoneSize = preset;
            UiHost.Configuration.Save();
    ...
    {
        DrawSizePills(winW, padX, t, "minisize", UiHost.Configuration.MiniPhoneSize, preset =>
        {
            UiHost.Configuration.MiniPhoneSize = preset;
            UiHost.Configuration.Save();
        ...
        ThemeDefinition t, string idPrefix, Action<PhoneScalePreset> onSelect)
    {
        ...
        ImGui.PushStyleVar(ImGuiStyleVar.FrameRounding, Px(8f));
        if (SharedUiHelpers.Button($"{label}##{idPrefix}{preset}", new Vector2(w, Px(30f))) && !selected)
        {
            onSelect(preset);
        }
        ImGui.PopStyleVar();
    ...
    private static void EnsurePreviewLogo()
    {
        ...
        {
            UiHost.Log.Warning(ex, "[AppearancePicker] Failed to load mini logo for preview.");
        }
    }
}
