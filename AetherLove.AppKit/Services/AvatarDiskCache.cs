...
/// <summary>Writes avatar bytes under a content-hashed filename to bust texture cache on content changes.</summary>
public static class AvatarDiskCache
{
    public static ISharedImmediateTexture? Store(string cacheDir, string key, byte[] bytes)
    {
        ...
        {
            UiHost.Log.Warning(ex, "[AvatarDiskCache] Could not store an avatar.");
            return null;
        }
    }
    ...
    private static void SweepStale(string cacheDir, string key, string keep)
    {
        foreach (var file in Directory.EnumerateFiles(cacheDir, $"{key}_*"))
        {
            ...
            {
                UiHost.Log.Warning(ex, "[AvatarDiskCache] Could not delete a stale avatar cache file.");
            }
        }
    }
}
