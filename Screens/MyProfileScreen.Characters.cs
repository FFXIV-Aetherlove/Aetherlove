...
public partial class MyProfileScreen
{
    ...
    private static string RpCharCacheDir =>
        Path.Combine(Plugin.PluginInterface.ConfigDirectory.FullName, "RpCharCache");
    ...
    private void OpenRpCharacters()
    {
        ...
        _rpLoadError = null;
        LoadRpCharacters();
        if (!Plugin.Configuration.SeenRpProfilesIntro)
        {
            Plugin.Configuration.SeenRpProfilesIntro = true;
    ...
    private void LoadRpCharacters()
    {
        ...
        _ = Task.Run(async () =>
        {
            ...
            catch (Exception ex)
            {
                ...
                Plugin.Log.Warning(ex, "[MyProfileScreen] GetMyCharactersAsync failed.");
            }
            finally
            {
                _rpLoading = false;
    ...
    {
        _selfieOverlay.Start(1.6f, PhotoSpec.PortraitWidth, (path, crop) => HandleRpImagePicked(row, path, crop));
    }
    ...
    private void SaveRpCharacters()
    {
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                ...
                // Adopt server ids by index so freshly-created rows can receive their image next.
            ...
            catch (Exception ex)
            {
                ...
                Plugin.Log.Warning(ex, "[MyProfileScreen] SaveRpCharacters failed.");
            }
            finally
            {
                _rpSaving = false;
