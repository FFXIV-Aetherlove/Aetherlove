...
public partial class ChatScreen
{
    ...
    // One fetched card per venue id shared into any chat (session cache); a null Card marks a venue that
    // could not be loaded (gone, or unreachable) so the row renders the unavailable placeholder.
    ...
    private void StartVenueCardFetch(Guid venueId)
    {
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                ...
                var cacheDir = Path.Combine(Plugin.PluginInterface.ConfigDirectory.FullName, "PlacesCache");
            ...
            {
                Plugin.Log.Warning(ex, $"[ChatScreen] Venue card fetch failed for {venueId}.");
    ...
    private void DrawVenueCardMessage(DisplayedMessage msg, Guid venueId, float windowWidth, bool isGroupEnd)
    {
        ...
        if (visual?.Card is { } card)
        {
            ...
            if (clicked)
            {
                ...
                _router.Navigate(Screen.Places);
            }
        }
        else
        {
