...
{
    private enum Section { Browse, Detail, AddVenue }
    ...
    private bool ProfileNsfwDefault => _bootstrap.LastConnection?.NsfwEnabled ?? false;

    private static string PlacesCacheDir =>
        Path.Combine(Plugin.PluginInterface.ConfigDirectory.FullName, "PlacesCache");

    public PlacesScreen(AetherLoveHubClient hubClient, SessionBootstrapper bootstrap,
        ScreenRouter router, ChatListScreen chatList, VenueShareContext shareCtx)
    {
    ...
    public void OnShow()
    {
        if (_shareCtx.PendingOpenVenueId is { } openVenueId)
        {
            _shareCtx.PendingOpenVenueId = null;
            OpenDetailFromChat(openVenueId);
        }
    ...
    private void StartBrowseFetch()
    {
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                var dto = await _hubClient.GetPlacesBrowseAsync(filter, ct).ConfigureAwait(false);
            ...
            {
                Plugin.Log.Warning(ex, "[PlacesScreen] Browse fetch failed.");
    ...
    private Vector2 DrawMenuButton(float winW)
    {
        ...
        var state = Plugin.Configuration.Places;
        if (state.TagMask != 0 || state.RegionMask != 0)
        {
    ...
    private void DrawMenuDropdown(Vector2 menuTL)
    {
        ...
        {
            var addVenueLabel = Loc.T("places.menu_addvenue");
            var w = MathF.Max(Px(150f), ImGui.CalcTextSize(addVenueLabel).X + Px(56f));
            var rowH = ImGui.GetTextLineHeight() + Px(12f);
            if (DrawMenuRow(FontAwesomeIcon.Filter, Loc.T("places.menu_filter"), w, rowH))
            {
                _showFilters = true;
                _filtersOpenedAt = ImGui.GetTime();
                ImGui.CloseCurrentPopup();
            }
            if (DrawMenuRow(FontAwesomeIcon.Search, Loc.T("places.menu_search"), w, rowH))
            {
                _searchActive = true;
                _focusSearch = true;
                ImGui.CloseCurrentPopup();
            }
            if (DrawMenuRow(FontAwesomeIcon.Store, addVenueLabel, w, rowH))
            {
                _section = Section.AddVenue;
                ImGui.CloseCurrentPopup();
            }
            var hiddenCount = Plugin.Configuration.Places.HiddenVenues.Count;
            if (hiddenCount > 0
                && DrawMenuRow(FontAwesomeIcon.EyeSlash, Loc.T("places.menu_hidden", hiddenCount), w, rowH))
            {
    ...
    private static bool DrawMenuRow(FontAwesomeIcon icon, string label, float w, float rowH)
    {
        var dl = ImGui.GetWindowDrawList();
        ...
        IconDraw.Add(dl, icon, iconPx, new Vector2(tl.X + Px(10f), tl.Y + (rowH - iconSz.Y) * 0.5f),
    ...
    private void DrawBrowseContent(PlacesBrowseDto browse)
    {
        ...
        var hidden = Plugin.Configuration.Places.HiddenVenues;
    ...
    private void DrawVenueContextMenu(VenueSummaryDto venue, string ctxId)
    {
        ...
        ImGui.Separator();
        if (ChatScreen.DrawIconMenuItem(FontAwesomeIcon.EyeSlash, Loc.T("places.hide_venue")))
        {
            ImGui.CloseCurrentPopup();
            HideVenue(venue);
        }
    ...
    {
        var places = Plugin.Configuration.Places;
