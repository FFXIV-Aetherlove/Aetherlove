...
public partial class PlacesScreen
{
    ...
    private void StartDetailFetch()
    {
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                var dto = await _hubClient.GetVenueDetailAsync(venueId, ct).ConfigureAwait(false);
            ...
            {
                Plugin.Log.Warning(ex, "[PlacesScreen] Venue detail fetch failed.");
    ...
    private static string RegionLabel(Shared.Profile.Enums.Region region)
    {
    ...
    private void ToggleLike(VenueDetailDto detail)
    {
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                var count = await _hubClient.SetVenueLikeAsync(venueId, liked, ct).ConfigureAwait(false);
            ...
            {
                Plugin.Log.Warning(ex, "[PlacesScreen] Like toggle failed.");
            }
            finally
            {
    ...
    private float DrawRsvpButton(VenueDetailDto detail, VenueOccurrenceDto occ, Vector2 topRight)
    {
        ...
        var clicked = ImGui.Button($"{label}##rsvp{occ.StartUtc.UtcTicks}", new Vector2(w, h));
        ImGui.PopStyleVar();
    ...
    private void ToggleRsvp(VenueDetailDto detail, VenueOccurrenceDto occ)
    {
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                var count = await _hubClient.SetVenueRsvpAsync(venueId, start, going, ct).ConfigureAwait(false);
                if (_detail is { } current && current.Summary.Id == venueId)
                {
                ...
                var fresh = await _hubClient.GetVenueDetailAsync(venueId, ct).ConfigureAwait(false);
                if (!ct.IsCancellationRequested && venueId == _detailVenueId)
                {
            ...
            {
                Plugin.Log.Warning(ex, "[PlacesScreen] RSVP toggle failed.");
            }
            finally
            {
                _engagementBusy = false;
    ...
    private void DrawReviewCompose(VenueDetailDto detail, float cardW)
    {
        ...
        if (ImGui.Button($"{submitLabel}##reviewSubmit", new Vector2(Px(150f), btnH)))
        {
            SubmitReview(detail);
        }
        if (!canSubmit)
        ...
        if (detail.MyReview is not null)
        {
            ...
            PushDangerButton();
            ImGui.PushStyleVar(ImGuiStyleVar.FrameRounding, Px(8f));
            if (ImGui.Button($"{Loc.T("places.review_delete")}##reviewDelete", new Vector2(Px(100f), btnH)))
            {
    ...
    private void SubmitReview(VenueDetailDto detail)
    {
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                await _hubClient.SubmitVenueReviewAsync(venueId, rating, text, ct).ConfigureAwait(false);
            ...
            {
                Plugin.Log.Warning(ex, "[PlacesScreen] Review submit failed.");
    ...
    private void DrawLoadMoreReviews(VenueDetailDto detail, float cardW)
    {
        ...
        ImGui.PushStyleVar(ImGuiStyleVar.FrameRounding, Px(8f));
        if (ImGui.Button(_moreReviewsLoading ? "…" : Loc.T("places.reviews_more"), new Vector2(cardW, Px(30f)))
            && !_moreReviewsLoading)
        {
    ...
    private void LoadMoreReviews(VenueDetailDto detail, int skip)
    {
        ...
        _ = Task.Run(async () =>
        {
            ...
            {
                Plugin.Log.Warning(ex, "[PlacesScreen] Review page fetch failed.");
            }
            finally
            {
    ...
    private void DrawDeleteReviewConfirm()
    {
                   ...
                   ImGuiWindowFlags.NoScrollbar | ImGuiWindowFlags.AlwaysUseWindowPadding))
        {
            if (child.Success)
            {
                ...
                PushDangerButton();
                ImGui.PushStyleVar(ImGuiStyleVar.FrameRounding, Px(8f));
                if (ImGui.Button(Loc.T("places.review_delete"), new Vector2(btnW, Px(30f))))
                {
                    _confirmDeleteReview = false;
                    DeleteMyReview();
                }
    ...
    private void DeleteMyReview()
    {
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                await _hubClient.DeleteMyVenueReviewAsync(venueId, ct).ConfigureAwait(false);
                _composeRating = 0;
                _composeText = "";
                if (venueId == _detailVenueId)
                {
            ...
            {
                Plugin.Log.Warning(ex, "[PlacesScreen] Review delete failed.");
            }
        }, ct);
    }
}
