...
public partial class ChatScreen
{
    ...
    private void ToggleMyReaction(Guid msgId, string emoji)
    {
        ...
        if (add)
        {
            Plugin.Configuration.ReactionUsage[emoji] =
                Plugin.Configuration.ReactionUsage.GetValueOrDefault(emoji) + 1;
    ...
    private void FireReaction(Guid msgId, string emoji, bool add)
    {
        ...
        _ = Task.Run(async () =>
        {
            ...
            {
                Plugin.Log.Warning(ex, "[ChatScreen] ReactToMessageAsync failed.");
    ...
    private void FirePin(Guid msgId, bool pinned)
    {
        ...
        _ = Task.Run(async () =>
        {
            ...
            {
                Plugin.Log.Warning(ex, "[ChatScreen] SetMessagePinnedAsync failed.");
    ...
    private static string[] ComputeQuickReact()
    {
        ...
        foreach (var name in Plugin.Configuration.ReactionUsage
                     .OrderByDescending(kv => kv.Value)
                     .Select(kv => kv.Key))
        {
            if (result.Count >= 6) { break; }
            if (result.Contains(name) || Plugin.EmojiService.GetEmoji(name) is null) { continue; }
            result.Add(name);
        }
