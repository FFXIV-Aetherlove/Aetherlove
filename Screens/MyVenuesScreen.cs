...
public partial class MyVenuesScreen
{
    ...
    private static string MyVenueCacheDir =>
        Path.Combine(Plugin.PluginInterface.ConfigDirectory.FullName, "MyVenueCache");

    private readonly Services.Auth.SessionBootstrapper _bootstrap;

    public MyVenuesScreen(AetherLoveHubClient hubClient, ScreenRouter router, ImageRequirementsModal imageReqModal,
    ...
    private void StartListFetch()
    {
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                var venues = await _hubClient.GetMyVenuesAsync(ct).ConfigureAwait(false);
            ...
            {
                Plugin.Log.Warning(ex, "[MyVenuesScreen] Venue list fetch failed.");
    ...
    private void DrawVenueCard(MyVenueDto venue, float winW)
    {
        ...
        // The reviews column: a divider, a star, and the "Reviews" label, centred.
