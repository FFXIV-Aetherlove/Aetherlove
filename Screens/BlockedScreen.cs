...
public sealed class BlockedScreen
{
    ...
    public BlockedScreen(ScreenRouter router, AetherLoveHubClient hub)
    {
        _router = router;
        _hub = hub;
    }
    ...
    private void StartFetch()
    {
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                ...
                var cacheDir = Path.Combine(Plugin.PluginInterface.ConfigDirectory.FullName, "BlockedAvatarCache");
            ...
            {
                Plugin.Log.Warning(ex, "[BlockedScreen] GetBlockedUsersAsync failed.");
    ...
    private void Unblock(BlockedUserDto target)
    {
        ...
        _ = Task.Run(async () =>
        {
            ...
            {
                Plugin.Log.Warning(ex, "[BlockedScreen] UnblockUserAsync failed.");
                _error = HubErrorText.Localize(ex);
            }
            finally
            {
