...
/// <summary>The Modern mode's skin assets, all loaded through the texture capability from the downloaded
/// <c>stacker</c> pack. One class for the three sheet roles: mino atlases
...
internal sealed class StackerArt(ITextureCache textures)
{
    ...
    /// <summary>The texture and pixel size for <c>stacker/&lt;role&gt;_&lt;skin&gt;.png</c>, falling back to
    /// <c>&lt;role&gt;_&lt;fallbackSkin&gt;.png</c>; null while missing or still decoding. A miss is not
    /// remembered, because the pack may still be downloading.</summary>
    public (ImTextureID Handle, Vector2 Size)? Get(string role, string skin, string? fallbackSkin = "default")
    {
        ...
        {
            path = MediaPaths.Downloaded(MediaPaths.Stacker, $"{key}.png");
            if (!File.Exists(path) && fallbackSkin is not null)
            {
                path = MediaPaths.Downloaded(MediaPaths.Stacker, $"{role}_{fallbackSkin}.png");
            }
            if (!File.Exists(path))
            {
                return null;
