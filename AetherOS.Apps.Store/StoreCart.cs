...
/// <summary>The shopping cart: product ids and quantities, persisted per install so it survives closing
...
internal sealed class StoreCart
{
    // The stored keys predate the rename to cart; changing them would empty every shipped cart.
    private const string CartKey = "bag";
    ...
    public StoreCart(IAppStorage storage) => _storage = storage;
    ...
    private void Load()
    {
        ...
        foreach (var line in _storage.Get<List<Line>?>(CartKey) ?? [])
        {
            if (line.Quantity > 0)
            {
                _lines.Add(line);
