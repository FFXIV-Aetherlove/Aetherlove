...
/// <summary>The cart: line items with steppers, the totals card, the insufficient-funds state with a way
/// to the Wallet, and the in-page confirm overlay that runs the sequential checkout loop. A failed line
/// stays in the cart with its typed error; bought lines leave it, because each purchase was real.</summary>
internal sealed class CartScreen(
    ...
    Action backHome, Action<SuccessScreen.Celebration> showSuccess, Action<OsAppContext> openWallet)
{
    ...
    private void Refresh()
    {
        ...
        _ = Task.Run(async () =>
        {
            ...
            foreach (var line in cart.Lines.ToArray())
            {
                ...
                {
                    cart.SetQuantity(line.ProductId, Math.Max(0, max - product.OwnedQuantity));
                }
            }
        });
    }
    ...
    private void DrawEmpty(OsAppContext ctx, float winW)
    {
        ...
        IconDraw.AddCentered(dl, FontAwesomeIcon.ShoppingCart, Px(38f),
        ...
        StoreFx.CenterLine(Loc.T("os.store_cart_empty"), winW, UiColors.Body);
        ImGui.Dummy(new Vector2(0f, Px(4f)));
        StoreFx.CenterWrapped(Loc.T("os.store_cart_empty_hint"), winW, UiColors.Hint, winW - (Px(PadX) * 2f));
    ...
    private void DrawLine(OsAppContext ctx, float winW, StoreCart.Line line)
    {
        var dl = ImGui.GetWindowDrawList();
        ...
        if (QuantityStepper.Draw($"##cart{line.ProductId:N}", stepperTl, 1, maxAddable, ctx.ReduceMotion, ref quantity)
    ...
    private void DrawTotals(OsAppContext ctx, float winW)
    {
        ...
        if (worth > total)
        {
            dl.AddText(new Vector2(x, y), ImGui.GetColorU32(UiColors.Hint), Loc.T("os.store_cart_discounts"));
        ...
        dl.AddText(new Vector2(x, y), ImGui.GetColorU32(UiColors.Body), Loc.T("os.store_cart_total"));
        ...
        var afterText = short_ > 0
            ? Loc.T("os.store_cart_short", short_.ToString("N0"))
            : Loc.T("os.store_cart_after", (balance - total).ToString("N0"));
        ...
        if (short_ > 0)
        {
            ...
            if (StoreUi.Button(Loc.T("os.store_cart_earn"), cardW - Px(28f)))
            {
                openWallet(ctx);
            }
            y += Px(38f);
    ...
    /// before the scrim). Confirm runs the sequential per-line checkout; a mid-cart failure keeps the
    ...
    private void DrawConfirmOverlay(OsAppContext ctx, float winW)
    {
        ...
        dl.AddText(new Vector2(x, panelTl.Y + Px(48f)), ImGui.GetColorU32(UiColors.Body),
            Loc.T("os.store_confirm_items", cart.Count));
