...
public sealed class PartsRig
{
    ...
    /// fields have to agree about. <see cref="EarFx"/> holds the argument in full.</summary>
    ...
    public void Update(float dt, bool reduceMotion, string mood, AtlasManifest? manifest, int cellIndex, float hopY256)
    {
        ...
        {
            // The clock wraps, and the schedule has to wrap with it: the same bookkeeping bug
            // EarFx carried, one class over. `nextSpellAt` is an absolute time on this clock, so
            // folding one and not the other left AmbientTail unable to advance after an hour of
            // uptime, and the tail held one spell's mood (Swish, every time) for the next hour.
            this.clock += dt;
            if (this.clock >= ClockWrap)
            {
                this.clock -= ClockWrap;
                this.nextSpellAt -= ClockWrap;
            }
        }
