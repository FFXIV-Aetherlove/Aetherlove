...
public static class GlyphDraw
{
    ...
    /// <summary>One glyph on its own, fitted to a square of <paramref name="side"/> centred on
    /// <paramref name="centre"/>: no frame, no envelope, no reveal. For a picker cell or a menu row,
    /// where the glyph IS the label and naming it would put words back on a wordless channel.</summary>
    public static void DrawIcon(ImDrawListPtr dl, Vector2 centre, float side, in GlyphShape shape,
        Vector4 ink, Vector4 fill)
    {
        if (side <= 0f)
        {
            return;
        ...
        Vector2 At(Vector2 p) => new(
            centre.X + (((p.X / 100f) - 0.5f) * side),
            centre.Y + (((p.Y / 100f) - 0.5f) * side));

        var fillCol = ImGui.ColorConvertFloat4ToU32(fill);
        var inkCol = ImGui.ColorConvertFloat4ToU32(ink);
        var weight = MathF.Max(1f, Stroke100 / 100f * side);
        ...
        for (var layer = 0; layer <= layers; layer++)
        {
            ...
            foreach (var path in shape.Paths)
            {
                ...
                StrokePath(dl, path, At, 1f, inkCol, weight * path.Weight);
            }
        }
    }
