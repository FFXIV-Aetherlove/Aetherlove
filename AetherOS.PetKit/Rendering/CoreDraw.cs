...
public sealed class CoreDraw
{
    ...
    /// <summary>Draws one worn accessory riding its anchor pin, so it follows hops and squashes
    /// with the body. Authored in 256-space: the sprite scales with display size, the slot's fit
    /// multiplier and the shell's per-item fit, never with the sheet's cell resolution. Head and
    /// face items squash with the pose; everything else stays rigid.</summary>
        ...
        HandFx? hands = null)
    {
        ...
        // around inside still turns with it. Every later way a pose learned to move has to be stood
        // down with it: this branch neutralises a pose by listing what a pose can do, so anything
        // added to PetPose that MOVES a pin has to be turned off here too.
        var still = def.StaysStill;
        var drawPose = pose;
        if (still)
        {
            drawPose.Scale = Vector2.One;
            drawPose.Offset = Vector2.Zero;
            drawPose.CellIndex = manifest.RestCell ?? pose.CellIndex;
            drawPose.NextCellIndex = drawPose.CellIndex;
            drawPose.FramePhase = 0f;
            drawPose.SmoothAnchors = false;
            drawPose.DrawnAnchor = null;
        }

        var local = flipX ? drawPose.Offset with { X = -drawPose.Offset.X } : drawPose.Offset;
        var anchorBase = bottomCentre + (local * (displaySize / 256f));
        ...
        var anchor = manifest.AnchorFor(def.Anchor, drawPose);
        if (seat is { } waist)
        {
        ...
        // The shell's own fit for this item: a nudge in 256-space accessory units, authored
        // unflipped, applied before the mirror so it swaps sides with the creature.
        var (fitScale, fitOffset, _) = manifest.FitFor(def.Slot, def.Name);
        anchor += fitOffset * (manifest.Cell / 256f);

        // An item pinned to a hand rides that limb: the limb's post-limit offset moves the pin
        // and its tilt leans the quad, so a held sword leans with the wave. With the limbs
        // disabled TryGet declines and items sit on their static pins exactly as before.
        var handTilt = 0f;
        if (hands is { Enabled: true } && hands.TryGet(def.Anchor, out var handOffset, out handTilt))
        {
            anchor += handOffset * (manifest.Cell / 256f);
        }
        ...
        var screenAnchor = LocalToScreen(manifest, anchor, drawPose.Scale, anchorBase, ds);
        ...
        var accessoryScale = (displaySize / 256f) * manifest.SlotScaleFor(def.Slot) * fitScale;
        var quadScale = def.Anchor is "head" or "face" || seat is not null
            ? new Vector2(accessoryScale) * reach * drawPose.Scale
        ...
        // A tilted item turns about its own pin. Authored unflipped, so the lean mirrors with
        // the creature.
        var angle = flipX ? -handTilt : handTilt;
        if (angle != 0f)
        {
            ...
            {
                var d = p - screenAnchor;
                return screenAnchor + new Vector2((d.X * cos) - (d.Y * sin), (d.X * sin) + (d.Y * cos));
            }

            drawList.AddImageQuad(
                handle,
                Turn(min), Turn(new Vector2(max.X, min.Y)), Turn(max), Turn(new Vector2(min.X, max.Y)),
                uv0, new Vector2(uv1.X, uv0.Y), uv1, new Vector2(uv0.X, uv1.Y),
                accessoryColour);
        }
        else
        {
        ...
        TentacleFx? strands = null)
    {
        ...
        {
            PartsDraw.DrawTail(drawList, manifest, def, pose, anchorBase, ds, palette, parts);
        }
        else if (def.Strands is { } worn)
        {
            PartsDraw.DrawEarStrands(drawList, manifest, def, worn, pose, anchorBase, ds, palette,
                strands ?? new TentacleFx());
        }
        else
        {
            PartsDraw.DrawEars(drawList, manifest, def, pose, anchorBase, ds, palette, parts);
        }
    }

    /// <summary>Both limbs of the Reaching, behind or in front of the body per the caller.
    /// Nothing draws for a manifest without both hand pins, or while the rig is disabled.</summary>
    ...
    {
        var manifest = _assets.Manifest;
        if (!manifest.Anchors.ContainsKey(HandFx.RightAnchor) || !manifest.Anchors.ContainsKey(HandFx.LeftAnchor))
        {
            return;
        }
        var ds = displaySize / manifest.Cell;
        var local = pose.FlipX ? pose.Offset with { X = -pose.Offset.X } : pose.Offset;
        var anchorBase = bottomCentre + (local * (displaySize / 256f));
        PartsDraw.DrawLimb(drawList, manifest, pose, anchorBase, ds, palette, HandFx.RightAnchor, hands, front);
        PartsDraw.DrawLimb(drawList, manifest, pose, anchorBase, ds, palette, HandFx.LeftAnchor, hands, front);
    }

    /// <summary>The shell's own strand anatomy (tendrils, legs, antennae), sown on the anchor the
    /// manifest names. Drawn behind the body: a shell that declares strands is anatomically
    /// incomplete without them, and nothing at all draws when it declares none.</summary>
    public void DrawShellStrands(
        ImDrawListPtr drawList,
        Vector2 bottomCentre,
        float displaySize,
        PetPose pose,
    ...
    {
        var manifest = _assets.Manifest;
        if (manifest.Strands is not { } def || !manifest.Anchors.ContainsKey(def.Seat))
        {
            return;
        }
        var ds = displaySize / manifest.Cell;
        var local = pose.FlipX ? pose.Offset with { X = -pose.Offset.X } : pose.Offset;
        var anchorBase = bottomCentre + (local * (displaySize / 256f));
        PartsDraw.DrawShellStrands(drawList, manifest, def, pose, anchorBase, ds, palette, rig);
    }
        ...
        KiteFx rig)
    {
        ...
        var ds = displaySize / manifest.Cell;
        var flipX = pose.FlipX;
        var local = flipX ? pose.Offset with { X = -pose.Offset.X } : pose.Offset;
        var anchorBase = bottomCentre + (local * (displaySize / 256f));
        var anchor = manifest.AnchorFor(def.Anchor, pose);
        if (flipX)
        {
            anchor.X = manifest.Cell - anchor.X;
        }
        var screenAnchor = LocalToScreen(manifest, anchor, pose.Scale, anchorBase, ds);
        ...
        float alpha = 1f)
    {
        ...
        var anchor = manifest.AnchorFor("mouth", pose);
        if (flipX)
        {
            anchor.X = manifest.Cell - anchor.X;
        }
        ...
        var ink = manifest.InkFor(bodyTint);
        MouthDraw.Draw(
            drawList, screenAnchor, displaySize / 256f, pose.Scale, flipX, shape, manifest.MouthScale, ink, alpha);
    }

    /// <summary>Any named anchor under the pose in screen space, squash and hop applied: what the
    /// glyph hangs from, exactly the mouth's own conversion made callable.</summary>
    public Vector2 AnchorScreen(string anchorName, Vector2 bottomCentre, float displaySize, PetPose pose)
    {
        var manifest = _assets.Manifest;
        var ds = displaySize / manifest.Cell;
        var flipX = pose.FlipX;
        var local = flipX ? pose.Offset with { X = -pose.Offset.X } : pose.Offset;
        var anchorBase = bottomCentre + (local * (displaySize / 256f));
        var anchor = manifest.AnchorFor(anchorName, pose);
        if (flipX)
        {
            anchor.X = manifest.Cell - anchor.X;
        }
        return LocalToScreen(manifest, anchor, pose.Scale, anchorBase, ds);
    }
