...
public partial class DeckScreen : IDisposable
{
    ...
    /// <summary>Set on an actual profile switch; the next deck apply drops the previous profile's card in hand and
    /// its reswipe history entirely (a switch is a clean slate, never a card-in-hand-preserving refresh).</summary>
    private volatile bool _profileSwitched;
        ...
        SettingsScreen settingsScreen)
    {
        ...
        _notifications.ProfileSwitched += OnProfileSwitched;
    }

    private void OnDeckRefreshRequested() => _forceRefresh = true;

    /// <summary>Fires on the switch's background thread, so it may only flip volatile flags: mark the pending
    /// full reset, force a refetch for the now-active profile, and drop any deck fetched for the previous one.</summary>
    ...
    public void Draw()
    {
        ...
        if (_pendingMatchNav)
        {
            ...
            _router.Navigate(LoveView.Match);
            return;
        }
    ...
    private void StartRefresh()
    {
        ...
        _ = Task.Run(async () =>
        {
            ...
            catch (Exception ex)
            {
                ...
                UiHost.Log.Warning(ex, "[DeckScreen] GetMatchDeckAsync failed.");
            }
            finally
            {
    ...
    private void ApplyPendingDeckIfReady()
    {
        ...
        // A profile switch is a clean slate: never pin the previous profile's card, and drop its reswipe history
        // so an undo can't resurrect the other profile's swipe. A normal refresh keeps the card in hand on top
        // unless the user left the deck while the slot elapsed.
        var switched = _profileSwitched;
        _profileSwitched = false;
        var pinned = (!switched && !_discardCurrentOnRefresh && _cards.Count > 0) ? _cards[0] : null;
        _discardCurrentOnRefresh = false;
        _processedThisPeriod.Clear();
        if (switched)
        {
    ...
    private void CachePortraits(IEnumerable<DeckCardDto> cards)
    {
        ...
        {
            UiHost.Log.Warning(ex, "[DeckScreen] Failed to create deck cache dir.");
            return;
        }
        ...
        foreach (var c in cards)
        {
            try
            {
                ...
                _portraitTextures[c.ProfileId] = UiHost.TextureProvider.GetFromFile(path);
            }
            catch (Exception ex)
            {
                UiHost.Log.Warning(ex, $"[DeckScreen] Failed to cache portrait for {c.ProfileId}.");
            }
        }
    }
    ...
    private void EnsurePortraitCached(DeckCardDto card)
    {
        ...
        try
        {
            ...
            if (!File.Exists(path))
            {
                ...
                _portraitTextures[card.ProfileId] = UiHost.TextureProvider.GetFromFile(path);
            }
        }
        catch (Exception ex)
        {
            UiHost.Log.Warning(ex, $"[DeckScreen] Failed to re-cache reswiped portrait for {card.ProfileId}.");
        }
    }
    ...
    private void CompleteSwipe()
    {
        ...
        _ = Task.Run(async () =>
        {
            ...
            {
                UiHost.Log.Warning(ex, $"[DeckScreen] SwipeAsync failed for {swipedCard.ProfileId}.");
            }
        });
    }
    ...
    private void OnReswipeClicked()
    {
        ...
        if (!UiHost.Configuration.SeenReswipeIntro)
        {
            UiHost.Configuration.SeenReswipeIntro = true;
        ...
        _ = Task.Run(async () =>
        {
            ...
            {
                UiHost.Log.Warning(ex, "[DeckScreen] UndoLastSwipeAsync failed.");
                _forceRefresh = true;
            }
        });
    }
