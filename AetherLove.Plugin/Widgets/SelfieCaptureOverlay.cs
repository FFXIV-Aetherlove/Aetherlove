...
public sealed class SelfieCaptureOverlay : Window
{
    ...
    private float _shotBoxH;

    private float BoxH => _freeForm ? _boxHFree : _boxW * _aspect;
    ...
    /// <param name="freeForm">No fixed aspect: the frame starts at 50% width / 80% height and resizes freely.</param>
    public void Start(float aspect, float minCropW, Action<string, Vector4> onDone, bool freeForm = false)
    {
        _aspect = aspect;
        _minCropW = minCropW;
        _freeForm = freeForm;
    ...
    public override void Draw()
    {
        ...
        if (_awaitingFrame)
        {
            DrawFrame(origin, size, _shotBoxTL, _shotBoxW, _shotBoxH);
        ...
        DrawFrame(origin, size, _boxTL, _boxW, BoxH);
    ...
    private void ClampBox(Vector2 size)
    {
        ...
        if (_freeForm)
        {
            _boxHFree = MathF.Max(MinBoxFloor, MathF.Min(_boxHFree, size.Y));
        }
        else if (BoxH > size.Y)
        {
    ...
    private void DrawFrame(Vector2 origin, Vector2 size, Vector2 boxTL, float boxW, float boxH)
    {
    ...
    private void HandleInput(Vector2 origin, Vector2 size)
    {
        ...
        if (_resizing)
        {
            ...
            var newW = MathF.Max(_minCropW, _resizeStartSize.X + delta.X);
            newW = MathF.Min(newW, size.X - _boxTL.X);
            if (_freeForm)
            {
                _boxW = newW;
                _boxHFree = Math.Clamp(_resizeStartSize.Y + delta.Y, MinBoxFloor, size.Y - _boxTL.Y);
            }
            else
            {
                _boxW = MathF.Min(newW, (size.Y - _boxTL.Y) / _aspect);
            }
            if (released)
            {
                _resizing = false;
        ...
        else if (clicked)
        {
            ...
            else if (overResize)
            {
                ...
                _resizeStartSize = new Vector2(_boxW, BoxH);
            }
            else if (overMove)
            {
                _movingBox = true;
