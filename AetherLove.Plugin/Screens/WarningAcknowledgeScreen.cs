...
public sealed class WarningAcknowledgeScreen
{
    ...
    /// <summary>Marks the next showing as a live mid-session push: on acknowledge it returns to where the
    /// user was instead of re-running the startup gate ladder. Called before the navigation to this screen,
    /// so the router still points at the interrupted location.</summary>
    ...
    {
        // A live mid-session warning returns to the interrupted location; re-running the startup ladder
        // would mis-route to gates.
        if (_liveReturn)
        {
            _liveReturn = false;
