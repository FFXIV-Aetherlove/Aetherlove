...
public sealed class ModeratorMessageScreen
{
    ...
    /// returns to where the user was instead of re-running the startup ladder. Called before the navigation
    /// to this screen, so the router still points at the interrupted location.</summary>
    ...
    public void Draw()
    {
        ...
        using (var scroll = ImRaii.Child("##modMsg", new Vector2(0f, scrollH), false))
        {
            ...
            var btnLabel = submitting ? Loc.T("common.acknowledging") : Loc.T("common.modmsg_got_it");
            if (submitting)
            {
                ImGui.BeginDisabled();
            }
